...
- Chrome拡張機能に接続できない場合 → すぐに以下をユーザーに提示してブラウザで直接開いてもらう（Chromeの確認は不要）：
  ...
  サーバー自体が起動していない場合のみ：
  「PowerShellで以下を実行してください：py "$PROJECT_DIR\local_runner.py"」
...
PowerShellウィンドウに『完了しました』と表示されたら教えてください。」
...
**STEP 2：リンク形式の確認（必ず以下の3択を提示すること）**

ユーザーに次の3択を選ばせる：

1. **URL挿入** — URLをそのまま挿入。WordPressのoEmbedでブログカード表示を試みる（表示結果はテーマ依存）
2. **aタグ** — テキストリンク形式。Gutenberg・クラシック両対応で確実にリンクが入る
3. **ショートコード** — カスタムショートコード形式。ターミナルでテンプレート文字列を入力してもらう
...
| 3. ショートコード | 1記事のみ | http://127.0.0.1:8765/run-wp?url=<URL>&link=shortcode&count=1 |
| 3. ショートコード | 一括 | http://127.0.0.1:8765/run-wp?url=<URL>&link=shortcode |
